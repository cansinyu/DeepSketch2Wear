...
#三维网格切割
def mesh_cut(mesh,region_rate=20,exp_weight=200): #region_rate：控制切割区域的大小。 exp_weight：用于调整面角度的权重系数。
    # remove small item 移除小区域
    cc = trimesh.graph.connected_components(mesh.face_adjacency, min_len=len(mesh.vertices)//5) #该区域的顶点数量应至少为网格顶点数的 1/5。
    mask = np.zeros(len(mesh.faces), dtype=bool)
    mask[np.concatenate(cc)] = True
    mesh.update_faces(mask)#更新面的索引
    mesh.remove_unreferenced_vertices()#移除未被引用的顶点
    points = np.sum(np.array(mesh.triangles), axis=1)#计算每个三角形的质心
    ptree = cKDTree(points)#建立三角形质心的 KD 树
    ...
    while True:
        ...
        for idx in near_idx:
            ...
            if idx not in seed_neighbor:
                # so this point may be a sink. 计算潜在的"水槽"区域
                idx_flag=True
                ...
                if len(sink_neighbor.intersection(seed_neighbor)) > 0:
                    break
                # create graph, use face angle as weight 构建图并计算最大流最小割
...
def smooth_border(filtered_mesh):
    ...
    for _ in range(10):
        ...
        laplacian = border_neighbouring_averages - filtered_mesh.vertices[border_vertices.astype(int)]
        border_vertices = border_vertices.astype(np.int32)
