...
def generate_template(factor: int) -> np.ndarray:
    """
    Generates a template array for voxel operations.

    Args:
        factor (int): The dimensionality factor of the template.

    Returns:
        np.ndarray: A template array with a shape of (factor**3, 3).
    """
...
def voxel2indices(voxel: np.ndarray, depadding: bool = False) -> np.ndarray:
    """
    Converts voxel data to indices.

    Args:
        voxel (np.ndarray): The input voxel data.
        depadding (bool, optional): Whether to perform depadding. Defaults to False.

    Returns:
        np.ndarray: An array containing voxel indices.
    """
...
def voxel2octree(voxel: np.ndarray, upfactor: int = 2, device: str = "cuda:0") -> Octree:
    """
    Converts voxel data into an Octree structure.

    Args:
        voxel (np.ndarray): The input voxel data.
        upfactor (int, optional): The upsampling factor. Defaults to 2.
        device (str, optional): The device to run on. Defaults to "cuda:0".

    Returns:
        Octree: The converted Octree object.
    """
...
def indices2octree(indices: np.ndarray, res: int = 32, upfactor: int = 2, device: str = "cuda:0") -> Octree:
    """
    Converts indices into an Octree.

    Args:
        indices (np.ndarray): The input index array.
        res (int, optional): The resolution. Defaults to 32.
        upfactor (int, optional): The upsampling factor. Defaults to 2.
        device (str, optional): The device to run on. Defaults to "cuda:0".
...
def octree2udfgrid(data: torch.Tensor, octree: Octree, depth: int, scale: float,
                   nempty: bool = False) -> np.ndarray:
    """
    Converts data from an Octree to a UDF (Unsigned Distance Field) grid.

    Args:
        data (torch.Tensor): The input Octree data.
        octree (Octree): The Octree object containing the Octree structure.
        depth (int): The depth of the Octree.
        scale (float): The scaling factor.
        nempty (bool, optional): Whether to process non-empty nodes. Defaults to False.

    Returns:
        np.ndarray: The converted UDF grid.
    """
...
def load_input(npy_path: str) -> np.ndarray:
    """
    Loads and processes .npy format voxel data.

    Args:
        npy_path (str): The path to the .npy file.

    Returns:
        np.ndarray: The binarized voxel array.
    """
