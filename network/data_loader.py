...
import numpy as np
import os
from utils.cloth_utils import snc_category_to_synth_id_all, snc_synth_id_to_category_all, TSDF_VALUE, snc_category_to_synth_id_5, snc_category_to_synth_id_13
...
class occupancy_field_Dataset(torch.utils.data.Dataset):
    def __init__(self, udf_folder: str, sketch_folder: str,
        ...
        if split_dataset:
            ...
            for _data_class in _data_classes:
                ...
                filelist = os.path.join(udf_folder, f"train_{label}.txt")
            ...
            txt_file_path = "./datasets/train.txt"
        ...
        self.alternate_feature_base = os.path.join(sketch_folder, "feature_aug")
        ...
        if self.use_sketch_condition:
            ...
            self.white_image_feature = np.load("utils/white_image_feature.npy")
    ...
    def __getitem__(self, index):
        ...
        if self.use_sketch_condition:
            if not self.detail_view:
                ...
                if random() < 0.5:
                    feature_path = os.path.join(self.feature_folder, f"{model_name}.npy")
                else:
                    random_edge_folder = np.random.choice(self.edges_folders)
                    feature_path = os.path.join(self.alternate_feature_base, model_name, random_edge_folder, "edge_front.npy")
                ...
                try:
                    image_feature = np.load(os.path.join(self.feature_folder, model_name))[sketch_view_index]
                    pm = np.load(os.path.join(self.pm_folder, model_name))[sketch_view_index]
                except Exception as e:
                    print(str(e))
                    image_feature = self.white_image_feature
